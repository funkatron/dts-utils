...
- **Browser UI:** **⌘↵** (macOS) or **Ctrl+Enter** runs **Generate** from the prompt field (hammer icon on **Generate**). **Setup** is the building FAB (top-right); **History** is the clock FAB stacked below it — lists recent successful generations stored only in **localStorage** (this browser); each row has **Download** links for PNGs. **Clear all** removes that storage.
