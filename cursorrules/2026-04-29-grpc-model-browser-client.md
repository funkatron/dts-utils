...
- Added restart-time model browser enablement with `dts-utils server restart --model-browser`.
- Preserved existing LaunchAgent settings by mutating only the installed service `ProgramArguments`.
- Added `dts-utils generate` (package `dts_utils.generate`) to call upstream streaming `GenerateImage` and write returned image bytes to disk.
...
- Promoted the useful reflection scratch probe to `dts-utils reflect`, then removed one-off scratch scripts.
...
- Added saved JSON generation configs through `dts-utils configs path/list`, plus `--configuration` auto-detection for `.json` files and saved config names.
- Renamed client-facing plaintext connection flag from `--insecure` to `--no-tls` to match the server install flag.
- Image generation is only `dts-utils generate` (removed the old `scripts/generate_image.py` path).
