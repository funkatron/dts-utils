...
SERVER_LIFECYCLE_SUBCOMMANDS = frozenset(
    {"install", "uninstall", "start", "stop", "restart", "test", "check", "status", "tail", "models"}
)
...
Lifecycle commands require the ``server`` prefix so they stay distinct from client RPC tools:
    ...
    {p} server models [--json]             List files the server exposes (Echo / model browser)
...
def main() -> None:
    ...
    if should_show_top_level_help(argv):
        ...
        sys.exit(0)
    if len(argv) >= 3 and argv[1] == "server" and argv[2] == "models":
